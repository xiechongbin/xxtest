...
public class MainActivity extends Activity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        VideoButton = findViewById(R.id.recoder);
    ...
    private void setupCamera(int width, int height) {
        CameraManager manager = (CameraManager) getSystemService(Context.CAMERA_SERVICE); //获取摄像头的管理者CameraManager
        try {
            for (String cameraId : manager.getCameraIdList()) { //遍历所有摄像头
                CameraCharacteristics characteristics = manager.getCameraCharacteristics(cameraId);
                Integer facing = characteristics.get(CameraCharacteristics.LENS_FACING);
                if (facing != null && facing == CameraCharacteristics.LENS_FACING_FRONT) //此处默认打开后置摄像头
                    continue;
                ...
                assert map != null;
                mPreviewSize = getOptimalSize(map.getOutputSizes(SurfaceTexture.class), width, height);  //根据TextureView的尺寸设置预览尺寸
                mCaptureSize = Collections.max(Arrays.asList(map.getOutputSizes(ImageFormat.JPEG)), new Comparator<Size>() {//获取相机支持的最大拍照尺寸
                    @Override
                    public int compare(Size lhs, Size rhs) {
                        return Long.signum(lhs.getWidth() * lhs.getHeight() - rhs.getHeight() * rhs.getWidth());
                    }
                });
                setupImageReader();  //此ImageReader用于拍照所需
                mCameraId = cameraId;
                break;
            }
    ...
    private Size getOptimalSize(Size[] sizeMap, int width, int height) {  //选择sizeMap中大于并且最接近width和height的size
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        try {
            if (resultCode == Activity.RESULT_OK && requestCode == 9999) {
                ...
                Log.e("recoder", "onActivityResult: " + uri.toString());
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
