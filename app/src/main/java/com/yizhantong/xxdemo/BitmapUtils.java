...
/**
 * @author Bitmap工具类主要包括获取Bitmap和对Bitmap的操作
 *         created at 2016/9/18 10:56
 */
public final class BitmapUtils {
    ...
    public static Bitmap createWatermarkBitmap(Bitmap src, Bitmap watermark) {
        ...
        // com.chexiaoya.wukong.common.network.store
        cv.restore();// 存储
        return newb;
    }
     ...
     * @return
     */
    public static Bitmap createCarBitmap(Context context, int resID, int bgColor, String carNum, int degree) {
        Bitmap srcBitmap = BitmapFactory.decodeResource
                (context.getResources(), resID);
        int canvasWidth = (int) (srcBitmap.getHeight() * 1.5);
        Bitmap destBitmap = Bitmap.createBitmap(canvasWidth, canvasWidth, Bitmap.Config.ARGB_8888);
        ...
        int padding = 7;
        int textWidth = (int) p.measureText(carNum) + padding;
        int textHeight = (int) Math.ceil(fm.descent - fm.ascent) + 2;
        int textBaseline = (canvasWidth - fm.bottom + fm.top) / 2 - fm.top;
        canvas.save();
        canvas.rotate(degree, canvasWidth / 2, canvasWidth / 2);
        canvas.drawBitmap(srcBitmap, new Rect(0, 0, srcBitmap.getWidth(), srcBitmap.getHeight())
                , new Rect((canvasWidth - srcBitmap.getWidth()) / 2, (canvasWidth - srcBitmap.getHeight()) / 2,
                        (canvasWidth + srcBitmap.getWidth()) / 2, (canvasWidth + srcBitmap.getHeight()) / 2), p);
        canvas.restore();
        canvas.drawRoundRect(new RectF((canvasWidth - textWidth) / 2, (canvasWidth - textHeight) / 2
                , (canvasWidth + textWidth) / 2, (canvasWidth + textHeight) / 2), padding, padding, pp);
        canvas.drawText(carNum, canvasWidth / 2, textBaseline, p);
    ...
    public static Bitmap ImageCropWithRect(Bitmap bitmap)
    {
        ...
        Bitmap bmp = Bitmap.createBitmap(bitmap, 0, 0, w, h/2, null,
            false);
